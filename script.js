...
function allowClick() {
    ...
    setTimeout(function () {
        // Redirect to the new date_page.html
        window.location.href = "date_page.html?name=" + encodeURIComponent(getURLParameter('name'));
    }, 500); // Adjust the delay as needed
}
...
function submitForm() {
    ...
    // Get value from the date and time input
    var dateTimeInput = document.getElementById("dateTimeInput");
    var dateTime = dateTimeInput.value; // No need to trim date and time
    ...
    // Redirect to home.html with the entered name and date/time in the URL after the fade-out
    setTimeout(function () {
        window.location.href = "home.html?name=" + encodeURIComponent(name) + "&dateTime=" + encodeURIComponent(dateTime);
    }, 500); // Adjust the delay as needed
}
